      ...
      setLoop(
        setInterval(() => {
          ...
          pathRef.current?.style.setProperty(
            ...
            pathRef.current?.getTotalLength().toString()
          );
          pathRef.current?.style.setProperty(
            'animation',
            'connection 0.65s infinite linear'
          );
          pathRef2.current?.style.setProperty(
            '--connection-path-lenght',
            pathRef2.current?.getTotalLength().toString()
          );
          pathRef2.current?.style.setProperty(
            'animation',
            'connection 0.85s infinite linear reverse'
          );
    ...
    <>
      <div {...rest} className="connection">
        ...
        <svg xmlns="http://www.w3.org/2000/svg">
          <path
            ref={pathRef}
            d={`M 10 80 Q 52.5 ${controlPoint}, 95 80 T 180 80`}
          />
          <path
            ref={pathRef2}
            d={`M 10 90 Q 52.5 ${controlPoint + 10}, 95 90 T 180 90`}
          />
        </svg>
      </div>

      <button
        onClick={() => (animation ? setAnimation(false) : setAnimation(true))}
      >
        CLICKKK
      </button>
    </>
